...
| 文本交互入口 | 官方 RKLLM 1.3.0 私有运行库，AArch64 原生构建 | PASS |
| D455 RSUSB | librealsense 2.56.5 私有构建，RGB-D 对齐短测 | PASS |
| 本地视觉问答界面 | GTK 画面、距离、提问和回答窗口 | DEPLOYED |

官方固定图片模型基线已通过，正式文本交互入口也已完成构建和动态库核查，尚未获得本轮模型执行批准。摄像头到位后的下一阶段为 D455 用户态采集基线，仍不得未经批准安装 DKMS、更新相机固件或执行主干合并。

...
{
    fprintf(stderr, "load lable %s\n", locationFilename);
    return readLines(locationFilename, label, OBJ_CLASS_NUM) == OBJ_CLASS_NUM ? 0 : -1;
}
...
static int process_i8_rv1106(int8_t *box_tensor, int32_t box_zp, float box_scale,
    ...
    fprintf(stderr, "validCount=%d\n", validCount);
    fprintf(stderr, "grid h-%d, w-%d, stride %d\n", grid_h, grid_w, stride);
    return validCount;
}
#endif
...
int post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold, object_detect_result_list *od_results)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        {
            fprintf(stderr, "RV1106/1103 only support quantization mode\n", LABEL_NALE_TXT_PATH);
            return -1;
        }
...
{
    const char *configured_path = getenv("YOLO_LABELS_PATH");
    const char *label_path = configured_path ? configured_path : LABEL_NALE_TXT_PATH;
